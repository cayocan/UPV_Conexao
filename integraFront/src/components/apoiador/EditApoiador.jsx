...
class EditApoiador extends Component{
	constructor(props){
		super(props)
		//console.log(props);
		this.state = {
...
function EditPaciComponent(props){
    ...
    function UpdateClicked(){
        // props.nav.try?
		IntegraServer.executeCreateApoia(values,sendDados)
			.then((response) => {
    ...
    function checkFields(){
        ...
        if(values.tipoDoador==='______'){
            setSnack({ mode:false,text:'Campo (Tipo de Apoaidor) vazio.',status:true });
            return            
        }
        if(values.funcNome===''){
    ...
    function setFromProps(){
        ...
        setValues({ ...values,
            ...
            empresa: props.nav.server.serverResponse.empresa,

            pessoaId:props.nav.server.serverResponse.pessoa.id,
            id:props.nav.server.serverResponse.id,
    ...
    function getSucessfulResponse(response){
        ...
        setValues({ ...values, 
            ...
            empresa: testNull(response.data.empresa,values.empresa),
    ...
    const clickAcompanhamentos=()=>{
		props.nav.push(`/apoia/${values.id}/doacoes`);
    }
    const clickAssociados=()=>{
		props.nav.push(`/apoia/${values.id}/contatos`);
    }
		...
		<Container maxWidth='lg' style={{marginBottom:'20px'}}>
			<ThemeProvider theme={ConstantStyles.MainTheme}>
				...
				<Paper style={{
						...
						<Typography style={{fontFamily:'Realist',flexBasis:'95%',color:'white',}}>{props.nav.try?'EDITANDO APOIADOR':'CRIANDO APOIADOR'}</Typography>
						<Typography style={{fontFamily:'Realist',color:'white',}}>{'APOIA0'+values.id}</Typography>
					</Paper>
					...
					<Box style={{width:'90%',}}>
                        ...
                        <Box style={{display:'flex',flexFlow:'row nowrap',justifyContent:'center',marginBottom:'60px'}}>
                            <ButtonGroup variant="outlined" color="primary" size="large">
                                ...
                                <Button onClick={clickAcompanhamentos}>Doações</Button>
                                <Button onClick={clickAssociados}>Contatos</Button>
                            </ButtonGroup>
                        </Box>:<></>}
						...
						<Grid container spacing={5} style={{width:'90%',justifyContent:'flex-end',}}>																					
                            ...
                            <LinhaMenuCampo name={"Tipo de Apoaidor:"} outro xs={11} defaultName={'tipoDoador'} onChange={handleValueChange} value={values.tipoDoador} arr={['Doador','Empresa','Profissional da Saúde','Acadêmico','Parcerias Diversas','Comunicação']} maxLength={32}/>
                            <LinhaCampo name={"Nome da Empresa / Instituição:"} xs={11} onChange={handleChange('empresa')} value={values.empresa} maxLength={64}/>
                            <LinhaCampo name={"Nome do Apoiador:"} need xs={11} onChange={handleChange('nome')} value={values.nome} maxLength={64}/>                            
...
export default connect(null, { push })(EditApoiador);