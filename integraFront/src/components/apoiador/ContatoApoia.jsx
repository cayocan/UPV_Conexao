...
class ContatoApoia extends Component{
	constructor(props){
		super(props)
		//console.log(props);
		this.state = {
	...
	requestServer(){
		IntegraServer.executeGetContatoById(window.location.pathname)
			.then((response) => {				
...
function ListMenu(props){
	...
	const[chamada,setChamada]=React.useState('Contato');
	...
	function handleRefresh(){
		//console.log('Refresh')
		handleCloseDialog()		
		IntegraServer.executeGetContatoById(state.urlId)
			.then((response) => {				
				handleSucessfulResponse(response)
	...
	function checkAcompSelected(parentState){		
		return <NovoContatoDialog apoiaId={state.urlId} parentState={parentState} handleClose={handleCloseDialog} handleRefresh={handleRefresh}/>
	}
	...
	return(
		<Container maxWidth='lg'>
                ...
                <Paper style={{
						...
						<Typography style={{fontFamily:'Realist',flexBasis:'95%',color:'white',}}>{'CONTATOS'}</Typography>
                ...
                <Box style={{display:'flex',flexFlow:'row nowrap',justifyContent:'center',marginBottom:'60px'}}>
                    <ButtonGroup variant="outlined" color="primary" size="large">
                        ...
                        <Button variant="contained" >Contatos</Button>
                    </ButtonGroup>
                </Box>
                ...
                <Box style={{display:'flex',flexFlow:'row nowrap',justifyContent:'space-between',alignItems:'center',marginBottom:'20px',height:'60px'}}>
					<Box>
						{//chamada!=''?
						<ConstantStyles.GreenButton onClick={handleClickOpenDialog} >Novo Contato</ConstantStyles.GreenButton>//:
                        /*<></>*/}												
					</Box>					
...
function ExpansionPanelList(props){
	...
	function checkAcompSelected(parentState,dados){
		return <NovoContatoDialog dados={dados} parentState={parentState} handleClose={handleCloseDialog} handleRefresh={props.handleRefresh}/>
	}	
	...
	return(
		...
		<ExpansionPanel expanded={expanded === 'panel1'} onChange={handleChange('panel1')} style={{boxShadow:'none',marginBottom:'10px'}}>
			<ConstantStyles.StyledPanelSummary>
				<Typography style={{fontFamily:'Realist',flexBasis:'95%',color:'white',}}>{props.obj.dataContato+' por '+props.obj.funcNome}</Typography>
				{<Typography style={{fontFamily:'Realist',color:'white',}}>{'CONTATO0'+props.obj.id}</Typography>}
			</ConstantStyles.StyledPanelSummary>
			<ConstantStyles.StyledPanelDetails>
                ...
                <DialogContent>
                    <LinhaInfo name={"Data do contato:"} data={props.obj.dataContato}/>					
                    <LinhaInfo style={{marginTop:'10px'}} name={"Tema:"} data={props.obj.tema}/>
                    <LinhaInfo style={{marginTop:'10px'}} name={"Observações:"} data={props.obj.obs}/>
			    </DialogContent>
...
function NovoContatoDialog(props){
	...
	function checkFields(){
        ...
        if(values.dataContato===null){
            setSnack({ mode:false,text:'Campo (Data do contato) vazio.',status:true });
            return 
        }
        if(values.tema ===''){
            setSnack({ mode:false,text:'Campo (Tema) vazio.',status:true });
            return
        }
        if(values.obs===''){
	...
	function SendData(){
		//props.nav.try?
		//console.log(props)
		IntegraServer.executeCreateApoiaContato(values,props)
			.then((response) => {
	...
	useEffect(() => {
		...
		if(props.dados){
			setValues({ ...values, dataContato:props.dados.dataContato, funcNome:props.dados.funcNome, obs:props.dados.obs, tema:props.dados.tema});
		}		
    }, [props.dados]);
	...
	return(
		<Box>
			...
			<DialogContent>				
				<LinhaCampo style={{marginBottom:'20px',}} name={"Data do contato:"} xs={11} onChange={handleDateChange('dataContato')} date value={values.dataContato}/>
				<LinhaCampo style={{marginBottom:'20px',}} name={"Responsável pelo contato:"} xs={11} onChange={handleChange('funcNome')} value={values.funcNome} maxLength={64}/> 
                <LinhaCampo style={{marginBottom:'20px',}} name={"Tema:"} xs={11} onChange={handleChange('tema')} value={values.tema} maxLength={32}/> 
...
export default connect(null, { push })(ContatoApoia);